...
| Script initialization time | Each script completes in ≤ 60 seconds on first run; ≤ 5 seconds on re-run | Container recovery must be fast. |
| Heartbeat update interval | Host bridge status file updated within 10 seconds of state change | Stale heartbeat = bridges appear dead. |
| Taygete max concurrent connections | ≥ 50 concurrent SSH probe connections without crash or resource exhaustion | Adversarial recon uses parallel probers. |
| Pleiades Nexus FIFO max latency | Events written within 1 second of trigger | Threat correlation requires near-realtime telemetry. |
| Pleiades Swarm broker decision latency | `.req` file processed within 2 seconds of creation | Policy decisions must not block service operations. |
| maia_crypto sign/verify latency | Round-trip ≤ 500ms for a 1 KiB message | Owner-escrow must not introduce operational delay. |
| Regression test suite runtime | `pleiades-regression.sh` completes in ≤ 5 minutes | Rapid iteration; CI gate must not block deploys. |
| Memory footprint per service | Each service uses ≤ 64 MiB RSS at idle | Container shares 4 GiB with 8 services + WSL overhead. |

## Dependencies

| Component | Runtime | Version / Source | Why |
|---|---|---|---|
| Gentoo nspawn container | systemd-nspawn ≥ 249 on WSL2 host | systemd package in Gentoo portage | All services run inside this boundary. |
| `maia_crypto` | Go binary (compiled at Maia.sh build time) | Go ≥ 1.21; `golang.org/x/crypto` ed25519 | Ed25519 sign/verify for owner-escrow signals. |
| `pleiades-request-broker` | Bash daemon or Go service | Bash ≥ 5.1 or Go ≥ 1.21 | Processes `.req` files from `/run/pleiades/requests/`. |
| Taygete `sandbox.js` | Node.js ≥ 18 LTS | Installed via `nvm` or system package | SSH honeypot with hostile recon detection. |
| `pleiades-regression-lib.sh` | Bash ≥ 5.1 | Sourced by test runner | Shared test helpers for all regression tests. |
| WSL2 host heartbeat | `pleiades-gentoo-heartbeat.sh` | Bash ≥ 5.1 (WSL2 host) | Manages host bridges; keeps container alive. |
| lm-sensors shim | Bash no-op function | Inline in each script | Prevents package manager errors on sensor queries. |
| `inotifywait` or polling fallback | `inotify-tools` ≥ 3.20 or busy-wait | Portage or fallback | Request broker watches `/run/pleiades/requests/`. |
| `jq` | JSON CLI processor ≥ 1.6 | Portage | Pleiades Swarm policy parsing and decision formatting. |
| `ssh` server for honeypot | OpenSSH ≥ 9.0 or custom listener | Node.js `ssh2` library v1.x | Taygete SSH decoy service. |
| `openssl` | OpenSSL ≥ 3.0 | System package | Fallback for key generation if maia_crypto unalcyoneilable. |
| systemd | systemd ≥ 249 | Container PID 1 | Service unit management; `systemctl is-active` checks. |
...
|---|---|---|
| E1 | Script run with `systemd` not yet PID 1 (bare chroot, no container) | Script detects missing systemd via `systemctl status` exit code; logs warning; exits non-zero; does NOT silently succeed. |
| E2 | `maia_crypto` binary missing at sign time | Maia.sh re-compiles from embedded Go source; if compilation fails, it exits non-zero and writes error to syslog. |
| E3 | `/run/pleiades/requests/` directory absent when broker starts | Broker creates directory with `install -d -m 700`; logs creation; resumes watching. |
| E4 | Pleiades Nexus FIFO removed mid-session | First script to write after removal re-creates it via `mkfifo`; appends event; logs re-creation. |
| E5 | Pleiades Swarm policy file absent at broker startup | Broker fails closed: denies all requests; logs `POLICY_MISSING` event to Pleiades Nexus FIFO; does not write a new policy file. |
| E6 | Taygete receives 200 rapid connections from non-loopback IP | Triggers rate-limit and permanent block after threshold (configurable, default 10/min); writes block event to FIFO; subsequent connections from blocked IP receive TCP RST. |
| E7 | Script re-run while service unit is already `active` | `systemctl start` is idempotent; script detects active status before calling start; skips start; exits 0 with "already active" log. |
| E8 | Container restarts and bridges are missing | Heartbeat detects missing bridge mounts within one heartbeat interval (≤ 10s); re-mounts them; writes `BRIDGE_REMOUNTED` event to FIFO. |
| E9 | Ed25519 key file tampered (wrong length or encoding) | `maia_crypto verify` returns non-zero; caller receives `SIGNATURE_INVALID`; no crash; no secret data in error output. |
| E10 | `pleiades-regression.sh` run with one service not active | Test function for that service emits `FAIL`; suite continues remaining tests; final exit code non-zero. |
